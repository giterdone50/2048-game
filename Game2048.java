...
public class Game2048 extends JPanel {
        ...
        if (!myWin && !myLose) {
          switch (e.getKeyCode()) {
            ...
            case KeyEvent.VK_DOWN:
            	down();
            	break;
            //inserted vk_up and vk_down and vk_right
  ...
  private boolean isFull() {
	  //call availableSpace here to complete this function
  ...
  boolean canMove() {
	...
	// call isfull 
	// first, check if full, if not, then return you can move 
	  if (isFull()==false) {
		  return true;
	  }
	// next, go through the whole board and check if value at a tile is same as the one adjacent to it
	// this check should be in an if statement inside a nested double for loop
	// if 2 elements are the same nearby, then return true
	for (int i=1; i < myTiles.length; i++) {
		for (int j=1; j < myTiles.length; j++) {
			Tile t = tileAt(i, j);
			if((t.value == tileAt(i+1, j).value) || (tileAt(i,j+1).value== t.value))  {
				return true;
			}
		}
	}
